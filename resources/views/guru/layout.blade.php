...
<head>
    ...
    {{-- PENTING: notifikasiGuru() HARUS sebelum
    <script defer> Alpine--}}
        <script>
            function notifikasiGuru() {
            return {
                buka: false,
            loading: false,
            total: 0,
            ...
            terakhirUpdate: '',
            intervalId: null,
            resetSiaUrl: '{{ route('guru.notifikasi.reset-sia') }}',
            ...
            getDismissed() {
                    try { return JSON.parse(sessionStorage.getItem('sinta_notif_guru_dismissed') || '[]'); } catch(e) { return []; }
                },

            saveDismissed(ids) {
                    try {sessionStorage.setItem('sinta_notif_guru_dismissed', JSON.stringify(ids)); } catch(e) { }
                },

            async ambilNotifikasi() {
                this.loading = true;
            try {
                        const res = await fetch('{{ route('guru.notifikasi') }}', {
            ...
            this.total = this.items.length;
            this.tutup();
            if (['sia_presensi','sia_nilai','sia_ekskul'].includes(item.id)) {
