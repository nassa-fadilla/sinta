    ...
    <div class="space-y-6">
        <section
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500">
                            Kirim pesan pertama kepada orang tua siswa binaan melalui WhatsApp.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6 p-5 md:p-6">
                ...
                <form method="POST" action="{{ route('guru.chat.store') }}" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <select name="owner_parent_id"
                            ...
                            @foreach($parents as $p)
                                @php
                                    $ortuNama = trim((string) ($p->nama_ayah ?? '')) !== ''
                                        ? $p->nama_ayah
                                        : (trim((string) ($p->nama_ibu ?? '')) !== ''
                                            ? $p->nama_ibu
                                            : ($p->display_name ?? 'Orang Tua'));

                                    $label = ($p->siswa_nama ?? 'Siswa') . ' • ' . $ortuNama;
                                    $hint = !empty($p->wa_target)
                                        ? ' [WA: ' . $p->wa_target . ']'
                                        : ' [WA tidak tersedia]';
                                @endphp

                                <option value="{{ $p->user_ortu_id }}" {{ old('owner_parent_id') == $p->user_ortu_id ? 'selected' : '' }}>
                                    {{ $label }}{{ $hint }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <p class="mt-1 text-[11px] text-slate-400">
                            Daftar ini hanya menampilkan orang tua dari siswa binaan Anda. Nomor WhatsApp tetap diambil
                            dari integrasi SIA.
                        </p>
                    </div>
