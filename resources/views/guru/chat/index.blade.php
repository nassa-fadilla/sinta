@extends('guru.layout')
@section('title', 'Riwayat Chat Orang Tua')
        ...
        <div class="space-y-6">
            <section
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div class="min-w-0">
                                <h1 class="text-xl font-semibold tracking-tight text-slate-800 sm:text-2xl">
                                    Riwayat Chat Orang Tua
                                </h1>
                                <p class="mt-1 text-sm text-slate-500">
                                    Kelola percakapan pribadi dengan orang tua siswa binaan melalui WhatsApp.
                                </p>
                            </div>
                        </div>

                        <div class="flex w-full flex-col gap-3 sm:flex-row lg:w-auto lg:items-center">
                            <form method="GET" action="{{ route('guru.chat.index') }}" id="chatSearchForm"
                                class="w-full sm:flex-1 lg:w-[14rem]">
                                <div
                ...
                <div class="overflow-hidden bg-white">
                    <div class="flex flex-col md:h-[calc(100vh-210px)] md:min-h-[500px] md:max-h-[620px] md:flex-row">
                                    ...
                                    <ul class="divide-y divide-slate-100 text-sm" id="threadList">
                                        @foreach ($threads as $thread)
                                            @php
                                                ...
                                                $fotoThread = $resolveSiswaFotoChat($thread);
                                            @endphp

                                            <li class="thread-item"
                                                data-search="{{ strtolower(($namaSiswa ?? '') . ' ' . ($namaOrtu ?? '')) }}">
                                                <a href="{{ route('guru.chat.show', $thread->id) }}"
                                                    ...
                                                    <div
                                                        ...
                                                        @if($fotoThread)
                                                            <img src="{{ $fotoThread }}" alt="Foto {{ $namaSiswa }}"
                        ...
                        {{-- PANEL KANAN --}}
                        <section class="flex min-h-[420px] flex-1 flex-col bg-white md:h-full md:min-h-0">
                            @if($activeThread)
                                @php
                                    ...
                                    $headerInitial = $makeInitial($activeThread->student_name ?? 'Siswa');
                                    $fotoHeader = $resolveSiswaFotoChat($activeThread);
                                @endphp
                                    ...
                                    <div class="flex items-start justify-between gap-3">
                                        <div class="flex min-w-0 items-center gap-3">
                                            <div
                                                ...
                                                @if($fotoHeader)
                                                    <img src="{{ $fotoHeader }}" alt="Foto {{ $activeThread->student_name ?? 'Siswa' }}"
                                                        class="h-full w-full object-cover object-top"
                                                        loading="lazy"
                                                        onerror="this.onerror=null; this.parentElement.innerHTML='<span>{{ $headerInitial }}</span>'; ">
                                                @else
                                    ...
                                    <div class="space-y-3">
                                        @forelse($messages as $m)
                                            @php
                                                ...
                                                $align = ($isWalkel || $isAdmin) ? 'justify-end' : 'justify-start';

                                                $bubble = ($isWalkel || $isAdmin)
                                ...
                                <div class="shrink-0 border-t border-slate-200 bg-white p-3 md:p-4">
                                    @if(($activeThread->status ?? 'open') === 'resolved')
                                        <div
                                            class="mb-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-sm text-amber-800">
                                            Percakapan ini sudah diselesaikan. Balasan baru tidak dapat dikirim.
                                        </div>
                                    @else
                                        @if(!$wa_target)
                                            <div
                                                class="mb-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-sm text-amber-800">
                                                Nomor WhatsApp orang tua belum tersedia. Balasan tetap dapat disimpan melalui sistem.
                                            </div>
                                        @endif

                                        <form action="{{ route('guru.chat.send', $activeThread->id) }}" method="POST"
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                function renderMessageBubble(msg) {
                    ...
                    const isWalkel = ['guru', 'wali', 'walkel'].includes(senderType);
                    const isAdmin = senderType === 'admin';
                    const isOutgoing = typeof msg.is_outgoing === 'boolean'
                        ? msg.is_outgoing
                        : (isWalkel || isAdmin);
                        ...
                        <div class="flex ${align} chat-message-item" data-message-id="${msg.id}">
                            <div class="max-w-[92%] sm:max-w-[85%] md:max-w-[76%]">
                                <div class="mb-1 flex flex-wrap items-center gap-2 text-[11px] text-slate-500">
                                    ...
                                    <span>•</span>
                                    <span>${escapeHtml(msg.time_label || msg.time || '-')}</span>
                                    ${statusBadge}
                                </div>
                ...
                async function fetchNewMessages() {
                    ...
                    try {
                        const url = `{{ route('guru.chat.fetchNewMessages', ['thread' => '__ID__']) }}`
                            .replace('__ID__', activeThreadId) + `?last_id=${lastId}`;
