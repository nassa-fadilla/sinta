<header
    ...
    @php
        ...
        $user = auth()->user();
        $role = strtolower((string) ($user?->role ?? 'walkel'));

        $namaUser = $user?->name ?? 'Wali Kelas';

        $jabatanUser = match ($role) {
            'walkel' => 'Wali Kelas',
            default => 'Guru',
        };
    ...
    <div class="max-w-screen-2xl mx-auto flex h-full items-center justify-between px-4 md:px-6">
        ...
        <div class="flex items-center gap-2">
            ...
            {{-- ═══ NOTIFIKASI WALKEL — Ikon Lonceng ═══ --}}
            <div x-data="notifikasiGuru()" x-init="init()" @click.away="tutup()" class="relative">
            ...
            <div x-data="{ open: false }" class="relative">
                ...
                <div x-show="open" x-transition x-cloak
                    ...
                    <div class="p-2.5">
                        ...
                        <form method="POST" action="{{ route('guru.logout') }}" class="mt-2">
                            @csrf
                            <button type="submit"
