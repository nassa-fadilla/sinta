...
@section('title', 'Master Mapel – SIA')@section('content')
    <div x-data="mapelIndex()" class="space-y-6">
        ...
        @php
            ...
            if ($kelompok) {
                $collection = $collection->filter(function ($m) use ($kelompok) {
                    return strtolower((string) ($m['kelompok'] ?? '')) === strtolower((string) $kelompok);
                })->values();
            }
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-sm text-slate-500">
                                    Gunakan pencarian dan filter untuk mempersempit data mata pelajaran.
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                            ...
                            <form method="GET" x-ref="searchForm" class="w-full lg:flex-1 lg:max-w-3xl">
                                ...
                                <div
                                    ...
                                    <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari nama mapel..."
                            ...
                            <form method="GET" class="flex w-full flex-col gap-3 sm:flex-row lg:w-auto lg:items-center">
                                ...
                                <select name="kelompok"
                                    ...
                                    @foreach (['Umum', 'Pilihan'] as $k)
                                        <option value="{{ $k }}" @selected(request('kelompok') == $k)>
                                            {{ $k }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <tbody class="divide-y divide-slate-100/90 text-slate-700">
                                @forelse($pageItems as $m)
                                    @php
                                        ...
                                        $dotClass = $status === 'aktif'
                                            ? 'bg-emerald-500'
                                            : 'bg-slate-400';

                                        $kelompokBadge = strtolower((string) ($m['kelompok'] ?? '')) === 'umum'
                                    ...
                                    <tr
                                        ...
                                        <td class="px-5 py-4">
                                            <span
                                                class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium {{ $kelompokBadge }}">
                                                {{ $m['kelompok'] ?? '-' }}
                                            </span>
                                        </td>
                                        ...
                                        <td class="px-5 py-4">
                                            <span
                                                ...
                                                <span>{{ $status === 'aktif' ? 'Aktif' : 'Nonaktif' }}</span>
                                            </span>
                                        </td>
                                        ...
                                        <td
                                            ...
                                            <a href="{{ route('kepsek.sia-master.mapel.show', $m['id']) }}"
